...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.channel.send(`<:MikKapal:866623408069804072> ${member.toString()} üyesi, \**${reason}\** nedeniyle susturuldu! **\(Ceza ID: #${penal.id})\** `);
    const time = ms(duration).replace("h", " saat").replace("m", " dakika").replace("s", " saniye");
    if (conf.dmMessages) member.send(`<:MikKapal:866623408069804072> **${message.guild.name}** sunucusunda, **${message.author.tag}** tarafından, **${reason}** sebebiyle, **${time}** boyunca susturuldunuz!`).catch(() => {});
